...
def main(args):
    ...
    if not is_delta_model:
        if generate_:
            inputs, outputs, test_inputs, test_outputs = generate_data(10000, is_fk=args.is_fk)
        else:
    ...
    for i in range(args.epoch):
        ...
        #     with torch.no_grad():
        #         losses.append(np.mean(batch_loss))
        #         test_x = torch.tensor(test_inputs, dtype = torch.float, requires_grad = False)
        #         test_y = torch.tensor(test_outputs, dtype = torch.float, requires_grad = False)
        #         prediction_y = fk_nn(test_x)
        #         test_loss = cost(prediction_y, test_y)
        #         # print(prediction_y.numpy().shape)
        #         # print(test_x.numpy().shape)
                
        #         if not args.is_fk:
        #             if args.generate_data:
        #                 predict_position = np.array([dobot.fk(prediction_)[:,-1][0:3] * 100 for prediction_ in prediction_y.numpy()])
        #                 # print(predict_position.shape)
        #                 dis, mean = distance(predict_position, test_x.numpy())
        #                 print("distance :", mean, " cm")
        #         else:
        #             dis, mean = distance(prediction_y.numpy(), test_y.numpy()[:,0:3])
        #             print("distance :", mean, " cm")
