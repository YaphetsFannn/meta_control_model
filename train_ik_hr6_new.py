...
def main(args):
    ...
    if args.d:
        ...
        plt.plot(range(len(dis_train)),dis_mean,color='g',label="training_set")
        ...
        print("mean dis:")
        print(dis_mean[-1])
        plt.plot(range(len(dis_test)),dis_mean,color='r',label=u"test_set")
        plt.xlabel(u"epoch")
        # plt.ylabel('mse loss')
        plt.ylabel(u'distance (cm)')
        plt.fill_between(range(len(dis_test)),
                        dis_min,
                        dis_max,
                        color='b',
                        alpha=0.2,label=u"range")
        plt.legend(loc='upper right', fontsize=20)
        plt.show()
        plt.clf()
        plt.plot(range(len(losses_test)),losses_test,label=u"training",color='r')
        plt.plot(range(len(losses_train)),losses_train,label=u"testing",color='g')
        plt.xlabel(u"epoch")
        plt.ylabel(u"lostfuction")
        plt.legend(loc='upper right', fontsize=20)
        plt.show()
    if args.s:
        PATH = "./model_trained/gim_"+args.name+".pkl"
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--s', type=bool, \
                            help='if load model', default=True)
